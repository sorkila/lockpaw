...
struct LockScreenView: View {
    ...
    var body: some View {
        GeometryReader { geo in
            ...
            ZStack {
                ...
                if !reduceMotion {
                    Circle()
                        .strokeBorder(Color("BevakaTeal").opacity(appeared ? 0 : 0.15), lineWidth: 1)
                ...
                VStack(spacing: 0) {
                    ...
                    ZStack {
                        if controller.unlockSucceeded {
                            ...
                            ZStack {
                                Ellipse()
                                    .fill(Color("BevakaTeal").opacity(0.02 + breathe * 0.02))
                                ...
                                Image("Mascot")
                                    ...
                                    .shadow(color: Color("BevakaTeal").opacity(0.15 + breathe * 0.08), radius: 35 + breathe * 8, y: 10)
                    ...
                    Group {
                        if controller.unlockSucceeded {
                            ...
                            Text(error)
                                ...
                                .foregroundStyle(Color("BevakaError"))
                                .shadow(color: Color("BevakaError").opacity(0.15), radius: 8)
                        } else if showMessage {
                            Text(message)
                                .font(.system(size: compact ? 14 : 16, weight: .regular))
                                .foregroundStyle(.white.opacity(0.55))
    ...
    private func background(geo: GeometryProxy) -> some View {
        ZStack {
            ...
            RadialGradient(
                colors: [Color("BevakaTeal").opacity(0.015 + breathe * 0.005), .clear],
    ...
    private func colorPools(geo: GeometryProxy) -> some View {
        ZStack {
            Circle()
                .fill(RadialGradient(colors: [Color("BevakaTeal").opacity(0.04 + breathe * 0.04), .clear], center: .center, startRadius: 0, endRadius: 300 + breathe * 40))
                .frame(width: 600, height: 600)
                .position(x: geo.size.width * 0.35 + drift * 10, y: geo.size.height * 0.3 + breathe * 8)
                .blur(radius: 80)

            Circle()
                .fill(RadialGradient(colors: [Color("BevakaAmber").opacity(0.02 + drift * 0.025), .clear], center: .center, startRadius: 0, endRadius: 250 + drift * 30))
